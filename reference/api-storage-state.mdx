...
Deletes all data in the scope, with optional exclusions.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `except_data` | Array of String | No | List of data keys to preserve. |
| `include_bot_data` | Boolean | No | Ignored (legacy parameter). |

> **Note:** Available on the `Account` scope only.
...
The `Account` object allows you to store data at the developer account level, making it accessible across **all bots** you own. 

You can use all standard KV methods on the `Account` object (`Account.saveData`, `Account.getData`, `Account.deleteData`, `Account.getDataFile`, `Account.getAllData`). 

```python BLP Example
# Save settings across all your bots
...
# Retrieve it from ANY of your bots

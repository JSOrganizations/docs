...
| `style` | String | No | Button color style (`primary`, `success`, `danger`, or `link`). |
| `icon_custom_emoji_id` | String | No | Premium custom emoji ID to display inside the button. |
...
```python BLP Example (Styled Inline Keyboard)
# Note: Bot Owner must have Telegram Premium to use Premium Custom Emojis.
# Button colors (styles) are available for all users.
EMOJI_ID = "5474667187258006816"
styled_keyboard = [
    [
        InlineKeyboardButton(text="Primary", callback_data="primary_btn", style="primary"),
        InlineKeyboardButton(text="Success", callback_data="success_btn", style="success")
    ],
    [
        InlineKeyboardButton(text="Danger", callback_data="danger_btn", style="danger")
    ],
    [
        InlineKeyboardButton(text="Emoji Button", callback_data="emoji_btn", icon_custom_emoji_id=EMOJI_ID)
    ]
]

Bot.sendMessage(
    chat_id=u,
    text="<b>🔥 New Button UI Preview</b>\n\nColored buttons + premium emoji icons",
    parse_mode="HTML",
    reply_markup=InlineKeyboardMarkup(inline_keyboard=styled_keyboard)
)
```
...
| `style` | String | No | Button color style (`primary`, `success`, `danger`). |
| `icon_custom_emoji_id` | String | No | Premium custom emoji ID to display inside the button. |
...
```python BLP Example (Styled Reply Keyboard)
# Note: Bot Owner must have Telegram Premium to use Premium Custom Emojis.
# Button colors (styles) are free for all users.
styled_reply_keyboard = [
    [
        KeyboardButton(text="Primary Button", style="primary"),
        KeyboardButton(text="Success Button", style="success")
    ],
    [
        KeyboardButton(text="Danger Button", style="danger")
    ],
    [
        KeyboardButton(text="Emoji Button", icon_custom_emoji_id="6080004495046089734")
    ]
]
...
Bot.sendMessage(
    ...
    parse_mode="HTML",
    reply_markup=ReplyKeyboardMarkup(
        keyboard=styled_reply_keyboard,
        resize_keyboard=True
    )
)
```
